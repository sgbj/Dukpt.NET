...
namespace Test
{
    class Program
    {
		...
		// Issue #7 added an example for IDTech decrypting
		static void Example_IDTech() {
			var test = "%B4266841088889999^BUSH JR/GEORGE W.MR^0809101100001100000000046000000?!";
            ...
            var decBytes = Dukpt.DecryptIDTech(bdk, ksn, BigInt.FromHex(track).GetBytes());
            var decrypted = UTF8Encoding.UTF8.GetString(decBytes);
			Console.WriteLine(decrypted);
            Console.WriteLine(decrypted == test);
